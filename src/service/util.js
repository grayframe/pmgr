	...
	async (actorID, objectID, newObject, {actionType='update', message='generic update'}) =>
{
	...
	let oldObject = await table
		.select(','.join(Object.keys(oldObject), 'id')
		.where('id', objectID);
...
const getChanges = (oldObject, newObject) =>
{
	...
	.filter(key => key !== 'id' && key !== history) //never record history for the id though i 
                                 //will prolly often pack it with the update object
	.forEach(key => 
	{
...
{
	let table = service[tableName];
	let histEntry = getBlankHistoryEntry({actionType, message, changes});
